...
// 환경 변수 파일의 값대로 로그 파일과 로그 패스를 가져오는 메인 함수 =>  매개변수 (환경 변수 파일, 로그 파일, 로그 패스)
void get_log_path(char env_file[], char log_file[], char log_path[]) {
  ...
  puts("\n    세팅에 필요한 파일을 입력해주세요.");
  printf("    >> ");
  fgets(env_file, FILE_NAME_SIZE, stdin);
  ...
  // 환경 변수의 key 값이 "file"이거나 "path"인 것을 찾기 위한 반복문 => 해당 key 값에 따른 value 값(writelog.txt)의 데이터를 읽기 위한 초기 과정
  int i = 0;
  while(read_len) {
    ...
    // 환경 변수의 key 값이 "file"이거나 "path"이면 해당 value를 log 변수에 저장
